...
class Game:
	...
	def events(self):
		for event in pg.event.get():
			...
			if event.type == pg.KEYDOWN: # Key Presses
				...
				if event.key == pg.K_SPACE and self.player.bombs > 0:
					self.player.bombs -= 1
					self.wipeEnemies()
				if event.key == pg.K_h:
					self.hardMode = True
	...
	def startScreen(self):
		...
		self.drawText("Press Any Key to Start", 30-TEXTDIFFERENCE, WHITE, WIDTH / 2, HEIGHT * 3 / 4, True)
		pg.display.flip()
		self.waitForKey()
